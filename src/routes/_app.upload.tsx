  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Drop an Excel or CSV file. We'll detect the columns and run a full analysis.</p>
      </div>
      ...
      <PCard>
        {state === "empty" && (
          <div
            ...
            <div className="text-lg font-semibold">Drag and drop a file here</div>
            <div className="mt-1 text-sm text-muted-foreground">or click to browse · .xlsx, .xls, .csv up to 50 MB</div>
            <button className="btn-primary mt-6">Choose file</button>
          </div>
        )}

        {state === "selected" && (
          <FileRow name="suppliers_april_2026.xlsx" size="284 KB" status="Ready to analyze" onCancel={reset}>
            <button onClick={analyze} className="btn-primary">Run analysis</button>
          </FileRow>
        )}

        {state === "analyzing" && (
          <FileRow name="suppliers_april_2026.xlsx" size="284 KB" status="Analyzing…" onCancel={reset}>
            <div className="inline-flex items-center gap-2 text-sm text-primary"><Loader2 className="h-4 w-4 animate-spin" /> Working</div>
          </FileRow>
        )}
        ...
        {state === "done" && (
          <div>
            <div className="flex items-center gap-3 rounded-xl border border-success/30 bg-success/10 p-4">
              ...
              <div className="flex-1"><div className="text-sm font-medium">Analysis complete</div><div className="text-xs text-muted-foreground">12 records · 8 columns · 2 anomalies detected</div></div>
              <Link to="/analysis" className="btn-primary">View results</Link>
            </div>

            <div className="mt-6 grid gap-4 md:grid-cols-2">
              <Info label="File name" value="suppliers_april_2026.xlsx" />
              <Info label="Rows analyzed" value="12" />
              <Info label="Detected columns" value="ID · Supplier · Category · Due Date · Amount · Status · Risk · Issue" />
              <Info label="Status" value="Completed" />
            </div>
            <div className="mt-6"><button onClick={reset} className="btn-ghost">Upload another</button></div>
          </div>
        )}
      </PCard>
      ...
      <PCard>
        ...
        <div className="mt-4 divide-y divide-border">
          {[
            ["payroll_q1_2026.xlsx", "Yesterday", "Completed"],
            ["expenses_march.csv", "3 days ago", "Completed"],
            ["vendors_master.xlsx", "Last week", "Completed"],
          ].map(([n,d,s]) => (
